...
const Sandbox: React.FC<SandboxProps> = ({ initialCode, isOpen, onClose, onDiscuss, width }) => {
  ...
  const [showSaveToLocalDialog, setShowSaveToLocalDialog] = useState(false);
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        await prettier.format(currentContent, {
            parser,
            // @ts-ignore - Prettier plugin types conflict
  ...
  const handleCommitToGitHub = async () => {
    if (!currentRepo || !commitMessage.trim()) {
      alert('Please provide a commit message');
      return;
    }
    
    setIsCommitting(true);
    setLogs(prev => [...prev, { level: 'info', args: ['Committing to GitHub...'], timestamp: new Date().toLocaleTimeString() }]);
    ...
    try {
      ...
      const sha = await githubService.getFileSha(currentRepo.owner, currentRepo.repo, fileName);
      ...
      setLogs(prev => [...prev, { 
        level: 'info', 
        args: [`✅ Committed ${fileName} to ${currentRepo.owner}/${currentRepo.repo}`], 
        timestamp: new Date().toLocaleTimeString() 
      }]);
  ...
  const handleSaveToLocal = () => {
    ...
    setLogs(prev => [...prev, { 
      level: 'info', 
      args: [`📥 Downloaded ${activeFile}`], 
      timestamp: new Date().toLocaleTimeString() 
    }]);
  };
  
  const handleLoadFromLocal = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setLogs(prev => [...prev, { 
        level: 'info', 
        args: [`📂 Loaded ${name} from local`], 
        timestamp: new Date().toLocaleTimeString() 
      }]);
    } catch (e: any) {
      setLogs(prev => [...prev, { 
        level: 'error', 
        args: [`Failed to load file: ${e.message}`], 
        timestamp: new Date().toLocaleTimeString() 
      }]);
    }
  };
      ...
      <div className="flex flex-col border-b border-slate-800 bg-slate-900 shrink-0">
        <div className="flex items-center justify-between p-2">
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
                ...
                <button onClick={handleSaveToLocal} className="flex items-center gap-1 px-3 py-1 bg-blue-900/20 text-blue-400 hover:text-blue-300 hover:bg-blue-900/30 rounded text-xs font-medium">
                    💾 Save
                </button>
                ...
                <input 
                  ...
                  accept=".html,.css,.js,.ts,.tsx,.json,.txt"
                />
                <button onClick={() => localFileInputRef.current?.click()} className="flex items-center gap-1 px-3 py-1 bg-yellow-900/20 text-yellow-400 hover:text-yellow-300 hover:bg-yellow-900/30 rounded text-xs font-medium">
                    📂 Load
                </button>
                
                {currentRepo && (
                  <button onClick={() => setShowCommitModal(true)} className="flex items-center gap-1 px-3 py-1 bg-green-900/20 text-green-400 hover:text-green-300 hover:bg-green-900/30 rounded text-xs font-medium">
                      🔼 Commit
                  </button>
                )}
      ...
      {showCommitModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 rounded-lg p-6 max-w-md w-full border border-slate-700">
            <h3 className="text-lg font-bold text-white mb-4">Commit to GitHub</h3>
            <p className="text-sm text-slate-400 mb-4">
              Committing <span className="text-cyan-400 font-mono">{activeFile}</span> to{' '}
              <span className="text-blue-400">{currentRepo?.owner}/{currentRepo?.repo}</span>
            </p>
            <textarea
              value={commitMessage}
              onChange={(e) => setCommitMessage(e.target.value)}
              placeholder="Commit message..."
              className="w-full bg-slate-800 text-white text-sm px-3 py-2 rounded border border-slate-700 focus:border-cyan-500 focus:outline-none mb-4"
              rows={3}
            />
            <div className="flex gap-2 justify-end">
              ...
              <button 
                ...
                className="px-4 py-2 bg-cyan-600 text-white rounded text-sm hover:bg-cyan-500 disabled:bg-slate-700 disabled:cursor-not-allowed"
              >
