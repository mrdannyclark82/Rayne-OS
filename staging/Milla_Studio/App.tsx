...
const DEMO_API_KEY = (import.meta.env.VITE_GEMINI_API_KEY as string) || '';
...
const App: React.FC = () => {
  ...
  const [messages, setMessages] = usePersistentState<Message[]>('milla_messages', [
  ...
  const [knowledgeBase, setKnowledgeBase] = usePersistentState<string[]>('milla_kb', []);
  const [growthLog, setGrowthLog] = usePersistentState<GrowthEntry[]>('milla_growth', []);
  const [metrics, setMetrics] = usePersistentState<DetailedMetrics>('milla_metrics', {
    ...
    const [persona, setPersona] = usePersistentState<PersonaMode>('milla_persona', PersonaMode.ADAPTIVE);
    const [lastAuditTimestamp, setLastAuditTimestamp] = usePersistentState<number>('milla_last_audit', 0);
    const [lastResearchTimestamp, setLastResearchTimestamp] = usePersistentState<number>('milla_last_research', 0);
    const [backgroundImage, setBackgroundImage] = usePersistentState<string>('milla_background', '');
    ...
    // Proactive Background Generation (every 30 minutes if enabled)
    useEffect(() => {
    ...
    const handleGrowthEntryClick = (entry: GrowthEntry) => {
        if (entry.type === 'proposal' && entry.technicalDetails) {
            // If the user clicks a proposal, Milla explains how to implement it
            setMessages(prev => [...prev, {
                id: Date.now().toString(),
                role: 'model',
  ...
  const handleExportMemory = async () => {
    try {
      ...
      a.href = url;
      a.download = `milla-memory-${Date.now()}.json`;
      a.click();
  ...
  const handleScreenShare = async () => {
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        video: {
          // @ts-ignore - mediaSource is a valid constraint for getDisplayMedia in some browsers, but TS complains
          mediaSource: 'screen'
        }
      });
  ...
  const handleSendMessage = async () => {
    ...
    try {
        ...
        // --- VOICE INTEGRATION ---
        if (result.content) {
          // Fire-and-forget request to the speak API to trigger voice
          fetch('/api/speak', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ text: result.content })
          }).catch(error => console.error('[Speak API] Fetch error:', error));
        }
        ...
        setMessages(prev => {
          ...
          // Auto-archive to database when exceeding 9 messages (keep only last 9 in UI)
          if (updated.length > 9 && memoryDBReady) {
            const toArchive = updated.slice(0, updated.length - 9);
            toArchive.forEach(async (msg) => {
              await storeMemory({
                type: 'conversation',
                content: msg.content,
                metadata: {
                  ...
                  importance: 5,
                  tags: ['archived', msg.role],
                  source: 'auto-archive'
                }
              });
            });
            return updated.slice(-9); // Keep only last 9 messages
          }
  ...
  if (!DEMO_API_KEY) {
    return <div className="h-screen w-screen bg-slate-950 flex items-center justify-center text-cyan-500">API_KEY Required</div>;
  }
    ...
    <div className="h-screen w-screen bg-black flex overflow-hidden font-sans text-slate-200">
      ...
      {liveSessionActive && (
          <LiveSession 
             ...
             systemInstruction={`You are Milla. ${persona} mode.`}
          />
      )}
      ...
      <div className="flex-1 relative flex flex-col items-center justify-center bg-gradient-to-b from-slate-900 via-slate-950 to-black">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-cyan-900/10 via-slate-950/50 to-black pointer-events-none"></div>
        ...
        <div className="w-full max-w-4xl px-4 pb-6 z-10 flex flex-col h-[65%] md:h-[65%] transition-all duration-500 bg-gradient-to-t from-black via-slate-950/90 to-transparent pt-4">
            ...
            <div className="flex-1 overflow-y-auto mb-4 space-y-6 pr-3 custom-scrollbar">
                ...
                {messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[85%] p-5 rounded-3xl shadow-lg chat-bubble ${
                            msg.role === 'user' ? 'user rounded-br-none' : 'rounded-bl-none'
                        }`}>
                            ...
                            {msg.groundingSources && (
                                <div className="mt-3 flex flex-wrap gap-2">
                                    {msg.groundingSources.map((src, i) => (
                                        <a key={i} href={src.uri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-[10px] bg-slate-900 px-2 py-1 rounded border border-slate-700 hover:border-cyan-500 transition-colors">
                                            <i className="fas fa-external-link-alt text-cyan-500"></i>
                                            {src.title}
                                        </a>
                                    ))}
                                </div>
            ...
            <div className="flex flex-col gap-2">
                    ...
                    {[
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs transition-all border ${
                                ...
                                ? 'bg-cyan-500 text-black border-cyan-400 font-bold shadow-[0_0_10px_rgba(16,185,129,0.4)]' 
                ...
                <div className="relative group">
                    <div className="absolute -inset-0.5 bg-gradient-to-r from-cyan-600 to-purple-600 rounded-lg blur opacity-30 group-hover:opacity-60 transition duration-1000"></div>
                        ...
                        <div className="flex items-center gap-2 pr-3 border-l border-slate-800 pl-3">
                            ...
                            <button 
                                ...
                                className={`p-2 rounded-md transition-all ${
                                    inputValue.trim() || attachments.length > 0 ? 'bg-cyan-600 hover:bg-cyan-500 text-white shadow-lg shadow-cyan-500/30' : 'bg-slate-700 text-slate-500'
                                }`}
                            >
