...
class GitHubService {
  ...
  async commitFile(owner: string, repo: string, path: string, content: string, message: string, sha?: string): Promise<void> {
    if (!this.token) {
      throw new Error('GitHub token required for commits');
    }
